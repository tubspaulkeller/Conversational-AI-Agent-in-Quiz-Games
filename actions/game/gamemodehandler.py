...
class GameModeHandler(BaseHandler):
    ...
    def __init__(self):
        ...
        self.leaderboard_collection = async_connect_to_db(self.db,'Leaderboard')
        self.competition_collection = async_connect_to_db(self.db, 'Competition')
    ...
    async def rate_open_question(self, answer, quest, retries=10):
        ...
        try:
            ...
            try:
                ...
                points = int(get_number_of_string.group()) if get_number_of_string else 0 
    ...
    def get_needed_time_for_answering(self, answer, id):
        ...
        question_timestamp, loop, quest_id ,_= timestamp_handler.get_timestamp(id, 'answer') 
    ...
    async def update_session(self, filter, name_of_slot, earned_points, group_answer, loop=None, mates_number = None):
        ...
        try: 
            ...
            level_points = [(1 , max_points - 53), (2, max_points - 43), (3,max_points -22), (4, max_points)]

            if loop and loop == 'KLOK':
                mates_number = mates_number
                avg_points = new_total_points / mates_number
                levels_achieved = [level for level, points in level_points if avg_points >= points]
            else:
                levels_achieved = [level for level, points in level_points if new_total_points >= points]
    ...
    def get_text_for_evaluation_of_points(self, is_user, level_up, kind_of_points, points_to_get):
        try:    
            if is_user:
                level_up_text = ""
                if level_up:
                    level_up_text = random.choice(["Außerdem bist du ein weiteres Level aufgestiegen!","Zusätzlich hast du ein weiteres Level erreicht!", "Darüber hinaus bist du auf eine höhere Stufe aufgestiegen!","Außerdem hast du dich in ein neues Level vorgearbeitet!","Des Weiteren hast du ein weiteres Level erfolgreich gemeistert!"])
                ...
                level_up_text = ""
                if level_up:
                    level_up_text = random.choice(["Außerdem seid ihr ein weiteres Level aufgestiegen!","Zusätzlich habt ihr ein weiteres Level erreicht!", "Darüber hinaus seid ihr auf eine höhere Stufe aufgestiegen!","Außerdem habt ihr euch in ein neues Level vorgearbeitet!","Des Weiteren habt ihr ein weiteres Level erfolgreich gemeistert!"])
    ...
    async def utter_points(self, solution, answer, points_to_get, level_up, recipient_id, is_user, name_of_slot, solution_points):
        ...
        try:
            ...
            if name_of_slot[-1] == "o":
                if points_to_get > 0:
                    text = self.get_text_for_evaluation_of_points(is_user, level_up, 'max', points_to_get)
                elif points_to_get == 0 and answer['answer']=='correct':
                    text= self.get_text_for_evaluation_of_points(is_user, level_up, 'less', points_to_get)
                elif points_to_get == 0 and answer['answer']=='wrong':
                    text = self.get_text_for_right_or_wrong_answer(False, solution, is_user=is_user)
            # Single/Multiple Choice Fragen
            else:
                if points_to_get > (solution_points * 0.75):
                    text = self.get_text_for_right_or_wrong_answer(True,  is_user=is_user) + self.get_text_for_evaluation_of_points(is_user, level_up, 'max', points_to_get)
                elif points_to_get > (solution_points/2):
                    text = self.get_text_for_right_or_wrong_answer(True,  is_user=is_user) + self.get_text_for_evaluation_of_points(is_user, level_up, 'middle', points_to_get)
                elif points_to_get == 0 and answer['answer']=='correct':
                    text = self.get_text_for_right_or_wrong_answer(True,  is_user=is_user) + self.get_text_for_evaluation_of_points(is_user, level_up, 'less', points_to_get)
                elif points_to_get == 0 and answer['answer']=='wrong':
                    text = self.get_text_for_right_or_wrong_answer(False, solution, solution_text = "Die Lösung lautet: ", is_user=is_user)
