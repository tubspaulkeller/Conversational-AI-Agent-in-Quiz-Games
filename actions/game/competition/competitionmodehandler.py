...
class CompetitionModeHandler(GameModeHandler):
        ...
        try:
            ...
            if name_of_slot[-1] == "o":
                ...
                # wrong answer
                    ...
                    if game_modus == 'quiz_form_KLOK' and username:
                        ...
                        wrong_answer_text = self.get_text_for_right_or_wrong_answer(False) 
                        wrong_answer_text = '@%s: '%(username) + wrong_answer_text +"\n"+ evaluation_text
            ...
            # SINGLE / MULTIPLE CHOICE
                ...
                if answer['answer'] == 'correct': 
                    ...
                    wrong_answer_text = self.get_text_for_right_or_wrong_answer(False, solution['solution'], solution_text="Die Lösung lautet: ", is_user=True)
                    wrong_answer_text = '@%s: '%(username)+ wrong_answer_text    
                    await self.telegram_bot_send_message('text', group_id, wrong_answer_text)
                elif answer['answer'] == 'wrong': 
                    wrong_answer_text = self.get_text_for_right_or_wrong_answer(False, solution['solution'], solution_text="Die Lösung lautet: ")
                    await self.telegram_bot_send_message('text', group_id, wrong_answer_text)
    ...
    async def calculate_points_competition(self, filter, name_of_slot,  id, points, answer, evaluation, game_modus):
        ...
        try: 
            username = answer['username'] if 'username' in answer else None
            result = self.get_needed_time_for_answering(answer, id)
            if name_of_slot[-1] == "o":
    ...
    async def calc_points_for_in_time_answering_competition(self, result, time_bound, max_points, penalty, name_of_slot, filter, evaluation, game_modus, username):
        try:
            ...
            if  result <= time_bound:
                ...
                print("\033[94m 1. Weniger als %s Sekunden gebraucht: %s, Punkte sind %s\033[0m" %(time_bound,result,points_to_get))
            ...
            # etwas zu spät geantwortet
                ...
                print("\033[94m 2. Weniger als %s Sekunden gebraucht: %s, Punkte sind %s\033[0m" %((time_bound+5),result, points_to_get))
            ...
            # zu spät geanwortet
                ...
                print("\033[94m 3. Länger als %s Sekunden gebraucht: %s, Punkte sind 0\033[0m" %(result, (time_bound+5)))
            return points_to_get
            ...
            for other in reversed(opponent['questions']):
                if other['id'] == name_of_slot:
                    ...
                    if game_modus == 'quiz_form_KLOK':
                        winner_options = self.get_winner_option_KLOK(my_group_points, other['points'])
                    #KLMK
                    else:       
                        winner_options = self.get_winner_option_KLMK(my_group_points, other['points'], answer['answer'], solution)
   
            winner = random.choice(winner_options) if len(winner_options) > 0 else "Ihr habt gewonnen!  🎉"
    ...
    def get_winner_option_KLOK(self,my_group_points, other_group_points):
        try:
            if my_group_points == 0 and other_group_points == 0:
                ...
                winner_options = [
                    "Ihr habt 0 Punkte geholt. Die andere Gruppe hat %s Punkte erreicht und gewinnt diese Runde." % (other_group_points),
                    "Ihr habt keine Punkte erzielt. Mit %s Punkten sichert sich die andere Gruppe den Sieg in dieser Runde." % (other_group_points),
                    "Ihr habt null Punkte erreicht. In dieser Runde erzielt die andere Gruppe %s Punkte und geht als Gewinner hervor." % (other_group_points)
            ]
            elif my_group_points > other_group_points:
                winner_options = [
                "Ihr habt diese Fragerunde gewonnen. 🎉\nIhr verdient euch %s Punkte und die andere Gruppe hat %s Punkte erzielt." %(my_group_points,  other_group_points ),
                "Ihr habt diese Fragerunde gewonnen! 🎉\nIhr erzielt %s Punkte, während die andere Gruppe %s Punkte erreicht hat." % (my_group_points, other_group_points),
                "Herzlichen Glückwunsch, ihr seid die Gewinner dieser Fragerunde! 🎉\nIhr verdient euch %s Punkte, während die andere Gruppe %s Punkte erzielt." % (my_group_points, other_group_points)
                ]
        
            elif my_group_points <  other_group_points:
                winner_options = [
                    "Die andere Gruppe hat mehr Punkte geholt. 😔 Ihr bekommt %s Punkte und die andere Gruppe hat %s Punkte erzielt." %(my_group_points,  other_group_points),
                    "Leider hat die andere Gruppe mehr Punkte erreicht. 😔 Ihr erhaltet %s Punkte, während die andere Gruppe %s Punkte erzielt hat." % (my_group_points, other_group_points),
                    "Die andere Gruppe hat mehr Punkte geholt. 😔 Ihr bekommt %s Punkte, während die andere Gruppe %s Punkte erzielt hat." % (my_group_points, other_group_points)
                ]
            else: 
                winner_options = [
                    "Beide Gruppen haben gleich viele (%s) Punkte für diese Frage erzielt. 🤝"%(my_group_points),
    ...
    def get_winner_option_KLMK(self, my_group_points, other_group_points, answer, solution):
        try: 
            ...
            # ich habe falsch geantworter, die andere Gruppe richtig
                ...
                winner_options = [
                    "Ihr habt die Frage leider falsch beantwortet. Die andere Gruppe hat sie richtig beanwortet.\nIhr bekommt 0 Punkte und die gegnerische Gruppe verdient sich %s Punkte. Bleibt weiter dran 💫"%(other_group_points),
                    "Leider habt ihr die Frage falsch beantwortet, während die andere Gruppe sie richtig beantwortet hat.\nIhr erhaltet keine Punkte, während die gegnerische Gruppe sich %s Punkte verdient. Weiter geht's! 💪" % (other_group_points),
                    "Eure Antwort auf die Frage war leider falsch, während die andere Gruppe sie richtig beantwortet hat.\nIhr erzielt keine Punkte, während die gegnerische Gruppe sich %s Punkte verdient. Weiter geht's! 😊" % (other_group_points)
                ]
    ...
    async def get_winner(self, group_id, group_id_opponent, dispatcher):
        ...
        try:
            ...
            if points_my_group > points_opponent:
                ...
                dispatcher.utter_message(text="Leider habt ihr mit %s:%s verloren 😔💔, aber ihr habt großartig gekämpft und es war ein spannendes Spiel."%(points_my_group,points_opponent))
            else:
                text = "Unternschieden"
                dispatcher.utter_message(text="Das Spiel endete unentschieden mit einem Ergebnis von %s:%s. 🤝 Es war ein hart umkämpftes Match, und beide Teams haben ihr Bestes gegeben."%(points_my_group,points_opponent))
            return text
    ...
    async def insert_leaderboard(self, session, loop, dispatcher, team_name, sender_id):
        try:
           ...
           # Aktuellen Unix-Zeitstempel erhalten
            ...
            # gen entry
            entry = {'Gruppenname': group_title, 'Punkte': total_points, 'Spieltag': unix_timestamp, 'Gamemode': loop}
    ...
    async def print_leaderboard(self, my_group, dispatcher, gamemode, sender_id):
        try:
            ...
            filtered_list = [entry for entry in leaderboard_list if entry.get('Gamemode', None) == gamemode]
            sorted_list = sorted(filtered_list, key=lambda x: x['Punkte'], reverse=True)
            ...
            if my_group not in best_entries:
                ...
                tab_data.append([place-1, place_before['Gruppenname'], place_before['Punkte'], self.format_date(place_before['Spieltag'])])
                
                #current group
                # dateformat "DD-MM"
                current_place = sorted_list[place-1]
                spieltag = self.format_date(current_place['Spieltag'])
                tab_data.append([place, current_place['Gruppenname'], current_place['Punkte'], spieltag])
                
                # group after
                if len(sorted_list) >= place + 1:
                    place_after = sorted_list[place]
                    tab_data.append([place+1, place_after['Gruppenname'], place_after['Punkte'], self.format_date(place_after['Spieltag'])])
            else:
