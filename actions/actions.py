...
class ValidateQuizFormKLOK(FormValidationAction):   
    ...
    def create_validation_function_for_slots_in_quiz_form_klok(name_of_slot):
        ...
        async def validate_slot(
            ...
            timestamp, loop, _,_ = timestamp_handler.get_timestamp(tracker.sender_id, 'waiting')
            ...
            if slot_value != "answered" and name_of_slot == get_requested_slot(tracker):            
                ...
                '''
                groupuser_id, groupuser_name, group_user_answer = get_groupuser_id_and_answer(tracker)
                ...
                create user answer
                '''
                user_answer = user_handler.created_user_answer(tracker.sender_id, name_of_slot, groupuser_id, groupuser_name, group_user_answer)
                ...
                '''
                klok_handler.process_user_answer(tracker.sender_id, user_answer, dispatcher)
                '''
                ...
                '''
                number_of_team_mates = competition_mode_handler.number_of_team_mates_slot(tracker, "my_group")
                klok_answers = klok_handler.get_klok_answers_for_group(tracker.sender_id)
                if not len(klok_answers) == number_of_team_mates: 
                    ...
                    if await competition_mode_handler.check_status('exceeded', name_of_slot, filter, competition_mode_handler.session_collection ):
                        dispatcher.utter_message(text="⏰😬 Ihr habt die Zeit überzogen und verliert das Spiel. 😔👎")
                        klok_handler.clear_group_answers(tracker.sender_id)
                        return{"answered": False, name_of_slot: "exceeded", "requested_slot": None}

                group_answers = klok_handler.get_klok_answers_for_group(tracker.sender_id)
                klok_handler.clear_group_answers(tracker.sender_id)

                print("GROUP_ANSWERS", group_answers)
                dispatcher.utter_message(response="utter_waiting_msg_evaluate_answers")
    ...
    def create_validation_function_for_slots_in_quiz_form_klmk(name_of_slot):
        async def validate_slot(
            ...
            if slot_value != "answered" and name_of_slot == get_requested_slot(tracker):
                ...
                create group answer
                '''
                group_answer = group_handler.created_group_answer(tracker.sender_id,name_of_slot, slot_value)
...
class ValidateQuizFormOKK(FormValidationAction):
    ...
    def create_validation_function_for_slots_in_quiz_form_okk(name_of_slot):
        async def validate_slot(
            ...
            create user answer
            '''
            user_answer = user_handler.created_user_answer(tracker.sender_id,name_of_slot, user["user_id"], user["username"], slot_value)
            if user_answer is None:
                return {name_of_slot: None}
