...
logger = logging.getLogger(__name__)
## actions
from actions.common.common import get_credentials,async_connect_to_db, ask_openai
                ...
                try: 
                    ...
                    if not 'callback_query' in request_dict:
                        ...
                        if request_dict['message']['from']['is_bot']== False:
                            ...
                            timestamp, loop, quest_id, opponent_id = timestamp_handler.get_timestamp(group_id, 'waiting')
                            text_message = request_dict['message']['text'] 
                            date = request_dict['message']['date'] 
                    

                    # Messages with @Ben pass through Rasa to call openai
                    if text_message and ('@Ben' in text_message or '@ben' in text_message):
                        request_dict['message']['text']  = '/ask_ben' #+ text_message
                    ...
                    # Messages with # do not ignore for answering questions
                        ...
                        if loop == 'quiz_form_KLOK' or loop == 'quiz_form_KLMK':
                            if "@Gegner" in text_message or "@gegner" in text_message:
                                ...
                                if "@Gegner" in text_message:
                                    new_text = text_message.replace("@Gegner", replacement)
                                elif "@gegner" in text_message:
                                    new_text = text_message.replace("@Gegner", replacement)
                                await out_channel.send_text_message(get_credentials(str(group_id)), new_text)
                        ...
                        if loop == 'quiz_form_KL' or loop == 'quiz_form_KLMK':
                            ...
                            if not existing_group:
                                self.collab_dict = {'group_id': request_dict['message']['chat']['id'], 'users' : []}
                            ...
                            for user in existing_group['users']:
                                if user['user_id'] == request_dict['message']['from']['id'] and user['group_id'] == str(request_dict['message']['chat']['id']) :
                                    ...
                                    break
                            # update in DB
                            update_counter = {"$set": {'users': existing_group['users']}}
                    ...
                    logger.exception("\033[91mException: %s\033[0m" %e)
            ...
            ######### Logic end ######## 
                ...
                try:
                    if update.callback_query:
                        ...
                        groupuser_name = str(group_user_cred["from"]["first_name"])

                    else:
                        group_user_cred = update
                        groupuser_id = str(group_user_cred["message"]["from"]["id"])
                        groupuser_name = str(group_user_cred["message"]["from"]["first_name"])
                except Exception as e:
                    logger.error(f"An error occurred while updating metadata: {str(e)}")
                ############
                ...
                if metadata is None:
                    ...
                    try:
                        ...
                        metadata.update({"groupuser_id": groupuser_id, "groupuser_name": groupuser_name})
                    except Exception as e:
                        logger.error(f"An error occurred while updating metadata: {str(e)}")
                else: 
                    try:
